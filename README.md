...
Streamlit은 `대화형 작업`, `형식 변환`, `라벨 생성`, `평가`, `설정`의 5개 탭으로 구성됩니다. 대화형 작업 탭에는 `현재 데이터셋의 라벨링 형식을 MS COCO 형식으로 바꿔줘`처럼 자연어로 작업을 요청할 수 있습니다. 라벨 생성 요청은 `검출`, `탐지`, `detect` 표현과 YOLO `names:` 클래스 블록, `path:` 이미지 경로를 해석할 수 있습니다. 변환과 평가는 workspace 안의 입력을 우선하지만, 라벨 생성은 프롬프트에 명시한 이미지 폴더가 workspace 밖이어도 입력으로 사용할 수 있으며 결과는 선택한 workspace의 `data/labeled`, `data/visualized`에 저장됩니다. 사용자가 `계획 실행`을 눌러 승인해야 실제 파일 생성 작업이 시작됩니다. 경로와 세부 옵션을 직접 지정해야 할 때는 해당 작업 탭을 사용합니다.
...
8. specialist 결과가 비어 있거나 `--generation_strategy vlm_first`이면 `LabelingAgent`가 저용량 모델로 같은 이미지를 `--inference_count`회 추론합니다. 이때 `--task_type`에 맞는 JSON 스키마를 LLM system prompt에 넣습니다.
9. `geometry.py`가 반복 결과의 consistency를 계산하고, 첫 번째 초안의 평균 confidence와 결합해 uncertainty를 계산합니다. detection은 bbox IoU, segmentation은 polygon mask IoU, pose는 keypoint 위치 기반 OKS-style 점수, OCR은 region IoU와 Unicode 문자 편집 유사도, classification과 tracking은 label 집합 Jaccard를 사용합니다.
10. consistency가 `--threshold`보다 낮으면 고용량 모델을 temperature `0.0`으로 한 번 더 호출하고, 아니면 첫 번째 저용량 초안을 최종 결과로 사용합니다.
11. 최종 결과를 `DatasetInsightAgent`에 누적하고, `LabelExportWriter`가 선택한 라벨 포맷으로 저장합니다.
12. 최종 결과를 원본 이미지 위에 그려 시각화 이미지를 저장하고, 이미지별 메트릭을 메모리에 누적합니다.
13. 모든 이미지 처리가 끝나면 `classes.txt`, YOLO `data.yaml`, COCO dataset JSON처럼 전체 실행 단위 산출물을 finalize합니다.
14. `run_metrics.csv`와 `run_metrics.jsonl`을 저장합니다.
15. `--gt_dir`가 있고 YOLO 출력이 포함된 경우에만 ground truth와 비교해 precision, recall, mean IoU를 출력합니다.
16. 전체 클래스 분포와 불균형 여부를 콘솔에 출력합니다.
