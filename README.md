...
Streamlit은 `대화형 작업`, `형식 변환`, `라벨 생성`, `평가`, `설정`의 5개 탭으로 구성됩니다. 대화형 작업 탭에는 `현재 데이터셋의 라벨링 형식을 MS COCO 형식으로 바꿔줘`처럼 자연어로 작업을 요청할 수 있습니다. 라벨 생성 요청은 `검출`, `탐지`, `detect` 표현과 YOLO `names:` 클래스 블록, `path:` 이미지 경로를 해석할 수 있습니다. 대화형 작업과 라벨 생성 탭의 라벨 생성은 먼저 1차 specialist 추론만 실행하고, 결과 리포트를 확인한 뒤 화면의 `Specialist 재추론 1회 실행` 버튼에서 `none`, `low`, `high`, `both` advisor를 선택해 검증 재추론을 이어서 실행할 수 있습니다. 형식 변환 탭은 `변환 사전 점검`으로 실행 전에 라벨 소스, 이미지 연결, YOLO class mapping 누락 여부를 확인할 수 있으며, 이미지 누락은 라벨 파일만 점검하는 사용 시나리오를 고려해 warning으로 표시합니다. 변환과 평가는 workspace 안의 입력을 우선하지만, 라벨 생성은 프롬프트에 명시한 이미지 폴더가 workspace 밖이어도 입력으로 사용할 수 있으며 결과는 선택한 workspace의 `data/labeled`, `data/visualized`에 저장됩니다. 사용자가 `계획 실행`을 눌러 승인해야 실제 파일 생성 작업이 시작됩니다. 경로와 세부 옵션을 직접 지정해야 할 때는 해당 작업 탭을 사용합니다.
