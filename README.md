...
- specialist-first 자동 라벨 생성 및 VLM fallback
- 실험 결과 평가
...
소스에서 GUI를 실행하거나 installer를 다시 빌드하는 방법은 [desktop_setup.md](desktop_setup.md)를 참고합니다. 기본 installer는 API 기반 기능과 라벨 변환·평가 의존성을 포함하고, 용량이 큰 `torch`, `transformers`, `ultralytics`, `paddleocr`, `easyocr` specialist 의존성 및 모델 가중치는 포함하지 않습니다. specialist 배포판을 만들 경우 기본 SAM backend는 Ultralytics SAM2(`sam2_b.pt`)이며, 공식 SAM3(`facebook/sam3`)는 Hugging Face gated 접근 권한이 있는 환경에서 선택적으로 활성화합니다.
...
Streamlit은 `대화형 작업`, `형식 변환`, `라벨 생성`, `평가`, `결과 리포트`, `설정`의 6개 탭으로 구성됩니다. 대화형 작업 탭에는 `현재 데이터셋의 라벨링 형식을 MS COCO 형식으로 바꿔줘`처럼 자연어로 작업을 요청할 수 있습니다. 라벨 생성 요청은 `검출`, `탐지`, `detect` 표현과 YOLO `names:` 클래스 블록, `path:` 이미지 경로를 해석할 수 있습니다. 대화형 작업과 라벨 생성 탭의 라벨 생성은 먼저 1차 specialist 추론만 실행하고, 결과 리포트를 확인한 뒤 화면의 `Specialist 재추론 1회 실행` 버튼에서 `none`, `low`, `high`, `both` advisor를 선택해 검증 재추론을 이어서 실행할 수 있습니다. 대화형 작업 결과는 `결과 리포트` 탭에서 확인하며, 결과 파일은 다운로드 버튼으로 받을 수 있습니다. 형식 변환 탭은 `변환 사전 점검`으로 실행 전에 라벨 소스, 이미지 연결, YOLO class mapping 누락 여부를 확인할 수 있으며, 이미지 누락은 라벨 파일만 점검하는 사용 시나리오를 고려해 warning으로 표시합니다. 변환과 평가는 workspace 안의 입력을 우선하지만, 라벨 생성은 프롬프트에 명시한 이미지 폴더가 workspace 밖이어도 입력으로 사용할 수 있으며 결과는 선택한 workspace의 `data/labeled`, `data/visualized`에 저장됩니다. 사용자가 `계획 실행`을 눌러 승인해야 실제 파일 생성 작업이 시작됩니다. 경로와 세부 옵션을 직접 지정해야 할 때는 해당 작업 탭을 사용합니다.

`data/labeled`는 AutoLabel이 새로 생성한 라벨을 저장하는 기본 출력 폴더입니다. `labels` 또는 `data/labels`는 외부 YOLO 데이터셋에서 자주 쓰는 관례적 입력 폴더명입니다. 대화형 변환은 `data/labeled`, `data/labels`, `labels`, `data/annotations`, `annotations`, `data/external_labels` 순으로 라벨 후보를 우선 탐색하고, 선택된 라벨과 가장 가까운 `data/raw`, `data/images`, `images` 계열 이미지 폴더를 연결합니다. 후보가 여러 개면 실행 계획의 확인 사항에 선택된 라벨/이미지 경로를 표시합니다.
...
| `--generation_strategy` | `specialist_first` | `specialist_first`는 specialist model 결과를 먼저 사용하고 비어 있을 때 VLM으로 fallback. `vlm_first`는 기존 VLM 초안 후 specialist 병합 |
| `--classes_path` | `None` | Grounding DINO와 SAM3 후보 클래스에 사용할 YOLO `data.yaml`, `dataset.yaml`, `classes.txt` |
| `--specialist_consistency_runs` | `0` | 1이면 1차 specialist 결과를 유지한 채 검증용 specialist 재추론을 1회 수행하고 agreement만 기록 |
| `--specialist_advisor_mode` | `none` | 재추론 설정 patch 제안 방식. `none`, `low`, `high`, `both`. LLM은 클래스와 bbox를 직접 수정하지 않음 |
...
라벨 생성은 기본적으로 태스크별 전문 모델 plugin을 먼저 실행하고, specialist 결과가 비어 있거나 `--generation_strategy vlm_first`를 지정한 경우 VLM cascade를 사용합니다. `--plugin_config`를 지정하지 않아도 기본 plugin chain이 생성되며, `configs/plugins.json` 또는 `--plugin_config`는 모델 checkpoint, 장치, threshold, weight 같은 세부 설정을 덮어쓰는 용도로 사용합니다. 빈 `plugins.json`이 있어도 기본 chain은 비활성화되지 않습니다.
...
| `object_detection` | `grounding_dino` | `--classes_path`, 프롬프트 `names:`, 설정 `labels` 기반 text-conditioned bbox 탐지 |
| `segmentation` | `grounded_sam2` | Grounding DINO bbox 탐지와 SAM2 polygon refinement를 하나의 pipeline plugin으로 실행 |
| `pose_estimation` | `pose` | Ultralytics pose 모델로 keypoint 생성 |
| `ocr` | `ocr` | PaddleOCR로 text region과 문자열 생성, 설정 시 EasyOCR backend 사용 가능 |
| `tracking` | `tracking` | ByteTrack 기반 track id 생성 |
...
전문 모델은 라벨 생성 태스크 시작 시점의 plugin prepare 단계에서 import와 weight 로드/다운로드를 먼저 시도합니다. 기본 segmentation 설정은 `grounded_sam2` plugin이 Grounding DINO와 Ultralytics SAM2(`sam2_b.pt`)를 내부 pipeline으로 로드합니다. 개별 `sam` plugin은 기본 비활성화되어 있으며 필요 시 `configs/plugins.json`에서 명시적으로 켤 수 있습니다. 공식 SAM3는 optional `sam` plugin에서 `official_sam3` backend와 `facebook/sam3` model로 바꿔 사용할 수 있지만 Hugging Face 모델 접근 승인과 로그인 토큰이 필요합니다. `requirements-specialists.txt`를 설치하지 않았거나 checkpoint 다운로드에 실패하면 기본값에서는 해당 오류를 `plugin_prepare_records`와 `plugin_records`에 기록하고 VLM fallback 가능 경로를 계속합니다. `--plugin_fail_fast`를 지정하면 즉시 중단합니다.
...
선택적으로 `--specialist_consistency_runs 1`을 지정하면 1차 specialist 결과를 최종 라벨 후보로 유지한 채 specialist 모델을 한 번 더 실행해 안정성만 측정합니다. `--specialist_advisor_mode`가 `low`, `high`, `both`이면 LLM은 `box_threshold`, `text_threshold`, `nms_iou`, `min_confidence`, prompt prefix/suffix, 제한적 augmentation 설정 patch만 제안합니다. 사용자가 지정한 클래스 목록은 immutable이며 LLM이 클래스 추가·삭제·이름 변경, bbox 생성, 최종 라벨 수정을 수행하지 않습니다. 결과는 `specialist_result_consistency`, `specialist_bbox_agreement`, `specialist_mean_matched_iou`, `specialist_rerun_patch`로 metric에 기록됩니다.

1차 specialist 추론 직후에는 이미지별 first-pass report를 생성합니다. 이 리포트에는 라벨 수, 클래스별 분포, confidence 평균/최소/최대, low-confidence 개수, plugin record, 현재 threshold 파라미터가 포함됩니다. `specialist_advisor_mode`가 LLM을 사용하는 경우 이 first-pass report가 advisor prompt에 참고 정보로 들어가며, 최종 라벨이나 bbox 수정 권한은 주지 않습니다.
...
6. 각 이미지마다 기본 `specialist_first` 전략으로 전문 모델 plugin을 먼저 실행합니다. 후보 클래스는 `--classes_path`, 프롬프트의 YOLO `names:` 블록, plugin 설정의 `labels` 순으로 사용됩니다.
7. specialist 결과가 있으면 VLM 호출 없이 validation/export 후보로 사용합니다.
8. 1차 specialist 결과에 대한 first-pass report를 생성해 `run_metrics.csv/jsonl`과 `run_summary.json`에 기록합니다.
9. `--specialist_consistency_runs 1`이면 report-only specialist 재추론을 수행하고 base/rerun bbox IoU agreement를 기록합니다. LLM advisor를 쓰는 경우 first-pass report를 참고 prompt로 전달합니다. 이 결과는 기본적으로 최종 라벨을 덮어쓰지 않습니다.
10. specialist 결과가 비어 있거나 `--generation_strategy vlm_first`이면 `LabelingAgent`가 저용량 모델로 같은 이미지를 `--inference_count`회 추론합니다. 이때 `--task_type`에 맞는 JSON 스키마를 LLM system prompt에 넣습니다.
11. `geometry.py`가 반복 결과의 consistency를 계산하고, 첫 번째 초안의 평균 confidence와 결합해 uncertainty를 계산합니다. detection은 bbox IoU, segmentation은 polygon mask IoU, pose는 keypoint 위치 기반 OKS-style 점수, OCR은 region IoU와 Unicode 문자 편집 유사도, classification과 tracking은 label 집합 Jaccard를 사용합니다.
12. consistency가 `--threshold`보다 낮으면 고용량 모델을 temperature `0.0`으로 한 번 더 호출하고, 아니면 첫 번째 저용량 초안을 최종 결과로 사용합니다.
13. 최종 결과를 `DatasetInsightAgent`에 누적하고, `LabelExportWriter`가 선택한 라벨 포맷으로 저장합니다.
14. 최종 결과를 원본 이미지 위에 그려 시각화 이미지를 저장하고, 이미지별 메트릭을 메모리에 누적합니다.
15. 모든 이미지 처리가 끝나면 `classes.txt`, YOLO `data.yaml`, COCO dataset JSON처럼 전체 실행 단위 산출물을 finalize합니다.
16. `run_metrics.csv`와 `run_metrics.jsonl`을 저장합니다.
17. `--gt_dir`가 있고 YOLO 출력이 포함된 경우에만 ground truth와 비교해 precision, recall, mean IoU를 출력합니다.
18. 전체 클래스 분포와 불균형 여부를 콘솔에 출력합니다.
