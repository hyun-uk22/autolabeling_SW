...
Streamlit 상단 대화창에는 `현재 데이터셋의 라벨링 형식을 MS COCO 형식으로 바꿔줘`처럼 자연어로 작업을 요청할 수 있습니다. 앱은 workspace에서 이미지와 라벨을 탐색하고 입력 포맷을 판별한 뒤 실행 경로와 출력 포맷을 계획으로 표시합니다. 사용자가 `계획 실행`을 눌러 승인해야 실제 파일 생성 작업이 시작됩니다. 경로와 세부 옵션을 직접 지정해야 할 때는 하단의 고급 작업 탭을 사용합니다.
