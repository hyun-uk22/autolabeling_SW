...
def _workspace_data_path(root: Path, relative_path: str) -> Path:
    path = Path(relative_path)
    parts = path.parts
    if root.name.lower() == "data" and parts and parts[0].lower() == "data":
...
def _explicit_named_path(
    ...
    patterns = (
        rf"(?im)^\s*(?:{name_pattern})\s*:\s*(.+?)\s*$",
        rf"(?im)(?:{name_pattern})\s*=\s*(.+?)(?:\s{2,}|$)",
    )
...
def _brief_notice_target(notice: Dict[str, Any], limit: int = 3) -> str:
    values = notice.get("files") or notice.get("examples") or []
    if not values:
        return "-"
    selected = [str(value).replace("|", "/") for value in values[:limit]]
    if len(values) > limit:
        selected.append(f"외 {len(values) - limit}건")
    return "<br>".join(selected)


def _brief_notice_action(notice: Dict[str, Any]) -> str:
    action = str(notice.get("user_action") or "").replace("|", "/")
    examples = notice.get("examples") or []
    if examples:
        first_sentence = action.split(". ", 1)[-1] if ". " in action else action
...
def build_conversation_plan(
    ...
    if action == "convert":
        ...
        explicit_path = (
            _explicit_workspace_path(source_path, root)
            if source_path
            else _explicit_named_path(
                request,
                root,
                ("label_path", "labels_path", "input_path", "label_dir", "labels_dir"),
            )
            or _explicit_workspace_path(request, root)
        )
        explicit_image_path = _explicit_named_path(
            request,
            root,
            ("image_path", "images_path", "img_dir", "image_dir", "images_dir"),
        )
        ...
        selected_images = (
            _image_directory_from_path(explicit_image_path, root)
            if explicit_image_path
            else _select_image_for_label(inventory["image_directories"], selected, root)
        )
        duplicate_iou = overrides.get("duplicate_iou")
        if duplicate_iou is None:
            duplicate_iou = _numeric_option(request, "iou")
        try:
            from ..reporting.conversion_preflight import build_conversion_preflight
            from ..utils.label_importer import import_labels_with_report

            preview_batch = import_labels_with_report(
                selected["path"],
            ...
            preflight_preview = build_conversion_preflight(preview_batch.report, formats)
        except Exception as exc:
            warnings.append(f"실행 전 입력 데이터 점검을 완료하지 못했습니다: {exc}")
        if len(candidates) > 1:
            warnings.append(
        ...
        operation = OperationPlan(
            ...
            out_dir=str(_workspace_data_path(root, "data/converted")),
            formats=formats,
            source_format="auto",
            duplicate_iou=duplicate_iou if duplicate_iou is not None else 0.85,
        ...
        operation = OperationPlan(
            ...
            vis_dir=str(_workspace_data_path(root, "data/visualized")),
            formats=formats,
        ...
        ground_truth = _workspace_data_path(root, "data/ground_truth")
        operation = OperationPlan(
            action="evaluate",
            gt_dir=str(ground_truth) if ground_truth.is_dir() else None,
            out_dir=str(_workspace_data_path(root, "data/reports")),
            runs=runs,
            require_approval=False,
        )
...
def describe_plan(proposal: Dict[str, Any], workspace: str | Path) -> str:
    ...
    preflight = proposal.get("preflight_preview") or {}
    notices = preflight.get("notices") or []
    if notices:
        lines.append("")
        lines.append("**실행 전 입력 데이터 확인사항:**")
        lines.append("")
        lines.append("| 심각도 | 변환 영향 | 항목 | 대상 데이터 | 확인/조치 |")
        lines.append("|---|---|---|---|---|")
        for notice in notices:
            lines.append(
                "| "
                f"{notice.get('severity', '-')} | "
                f"{notice.get('conversion_impact', '영향 미확인')} | "
                f"`{notice.get('code', '-')}`<br>{notice.get('message', '')} | "
                f"{_brief_notice_target(notice)} | "
                f"{_brief_notice_action(notice)} |"
            )
