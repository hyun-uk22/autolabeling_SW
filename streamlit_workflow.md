...
한국어 또는 영어 자연어 요청을 처리한다. 형식 변환, 라벨 생성, 실험 평가는 구조화된 `WorkflowPlan`으로 변환하고, 라벨 편집 요청은 별도 UI route(`open_editor`)로 처리해 라벨 편집 탭을 연다.
...
`evaluate_labels`, `open_label_editor` 등의 제한된 intent와 포맷, task, threshold 같은 구조화된 값만 반환한다.
...
대화형 라벨 생성은 초기 프롬프트로 재생성 비교 여부를 바로 확정하지 않는다. 먼저 1차 specialist 추론 결과와 리포트를 확인한 뒤, 결과 리포트 화면에서 `LMM 재생성 비교`를 선택적으로 실행한다. LMM 재생성 비교는 1차 Vision Model 결과와 Low/High LMM 결과 간 bbox IoU 기반 self_consistency를 계산하고, 임계치 미달 이미지만 라벨 편집 큐로 전달할 수 있다.

대화형 라벨 편집 요청은 실행 계획 승인 단계를 거치지 않는다. workspace에서 라벨 후보와 이미지 후보를 자동 선택해 라벨 편집 탭의 입력값을 채우고, 가능한 경우 라벨을 자동으로 불러온다.
...
- 대화형 라벨 생성의 1차 실행은 LMM 재생성 비교 없이 시작하며, Low/High LMM 비교는 1차 결과 확인 이후 UI에서 선택
- 중복 IoU
- strict 검증 제외 여부
- 생성 threshold
- 복수 출력 포맷
...
사용자가 `계획 실행`을 선택해야 실제 operation이 실행된다. `취소`를 선택하면 파일을 변경하지 않는다. 계획이 대기 중일 때도 채팅 입력으로 경로, 출력 포맷, 태스크 등을 수정할 수 있으며, 수정안은 검증 후 pending plan에 반영된다.
...
| 클래스 매핑 파일 | Grounding DINO, `grounded_sam2` wrapper, classification 후보 클래스에 사용할 YOLO `data.yaml`, `dataset.yaml`, `classes.txt` | 선택 |
...
라벨 생성 탭도 대화형 작업과 동일하게 초기 실행에서는 LMM 비교를 수행하지 않는다. 1차 결과 리포트를 확인한 뒤 결과 화면 아래에서 LMM 재생성 비교를 선택적으로 이어서 실행한다. LMM 비교에서 self_consistency 임계치 미달로 판정된 이미지는 통과 이미지와 분리해 라벨 편집 탭으로 넘길 수 있다.
...
이미지 목록과 모델 설정 검증
  ...
  -> 선택 시 Low/High LMM 재생성 및 base/LMM bbox self_consistency 계산
...
Specialist 재추론은 runtime 옵션으로 남아 있는 report-only 검증 경로다. Streamlit 기본 흐름에서는 1차 Vision Model 결과를 유지한 뒤 사용자가 선택할 경우 LMM 재생성 비교를 실행한다. 기본 object detection은 Grounding DINO 단독, 기본 segmentation은 코드의 `grounded_sam2` wrapper(Grounding DINO + Ultralytics SAM2)를 사용한다. Advisor LMM을 사용하는 실험 경로에서는 선택한 LMM이 threshold/prompt/augmentation 설정 patch만 제안하며 클래스 목록, bbox, 최종 라벨은 직접 수정하지 못한다.
...
1차 specialist 리포트는 `first_pass_report`, `first_pass_total_labels`, `first_pass_mean_confidence`, `first_pass_low_confidence_count`로 저장된다. LMM 재생성 비교를 실행하면 1차 라벨은 기존 출력 폴더에 유지되고, Low/High LMM 라벨은 `<라벨 출력>_llm/low`, `<라벨 출력>_llm/high`에 별도 저장된다. self_consistency 임계치 미달 후보는 `<라벨 출력>_review/low`, `<라벨 출력>_review/high`에 분리 저장된다.
...
대화형 작업에서 라벨 편집을 직접 요청하면 자동 선택된 라벨/이미지 경로로 라벨 편집 탭을 열 수 있다. 또한 형식 변환, 라벨 생성, 결과 리포트에서 문제가 있는 파일을 선택하면 해당 파일만 라벨 편집 큐로 전달할 수 있다. LMM 재생성 비교의 경우 self_consistency 임계치 미달 이미지만 큐에 들어가며, 통과 이미지는 편집 목록에서 제외된다.
