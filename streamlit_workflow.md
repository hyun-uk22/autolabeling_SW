...
변환 리포트에는 실제 감지된 입력 포맷, 읽은 레코드 수, 변환 성공 수, validation 요약, 레코드별 이슈와 artifact 경로가 포함된다. 수동 형식 변환 탭은 operation output 전체를 Streamlit JSON으로도 표시한다.
...
`run_summary.json`에는 task, `consistency_metric`, 포맷, 처리 이미지 수, 총 라벨 수, 실행 시간, low/high API 호출 수, escalation 수와 artifact 경로가 기록된다. 이미지별 지표에도 사용한 `consistency_metric`과 label 종류별 개수, consistency, confidence, uncertainty, plugin 결과, validation issue와 시각화 경로가 포함된다.
...
각 수동 작업 탭은 완료 상태, operation outputs, errors, history path를 Streamlit JSON으로 표시한다. 대화형 작업 탭은 같은 결과에서 핵심 항목만 문장으로 요약한다.

리포트는 UI에만 존재하는 임시 데이터가 아니라 각 operation의 출력 디렉터리에 파일로 저장된다. 현재 UI는 결과 파일 경로와 JSON을 제공하지만 별도의 리포트 전용 표·차트 화면이나 다운로드 버튼은 제공하지 않는다.
