...
Specialist 재추론은 report-only 검증 단계다. 1차 Grounding DINO/SAM2 결과를 최종 라벨 후보로 유지하고, threshold/prompt/augmentation 설정을 조금 바꾼 specialist 결과와 bbox IoU agreement를 계산한다. Advisor가 `low`, `high`, `both`이면 선택한 LLM은 설정 patch만 제안하며 클래스 목록, bbox, 최종 라벨은 직접 수정하지 못한다.
