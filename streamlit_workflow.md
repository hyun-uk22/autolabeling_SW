...
대화형 라벨 생성은 초기 프롬프트로 재추론 여부를 바로 확정하지 않는다. 먼저 1차 specialist 추론 결과와 리포트를 확인한 뒤, 화면의 `Specialist 재추론 1회 실행` 영역에서 `none`, `low`, `high`, `both` advisor를 선택해 선택적으로 검증 재추론을 실행한다.
...
`변환 사전 점검` 버튼은 파일을 생성하지 않고 라벨 소스 탐색, YOLO class mapping 탐색, 이미지 연결, validation issue를 먼저 확인한다. 사용자가 라벨 파일만 가져와 점검할 수 있도록 이미지 파일 연결 실패는 warning으로 표시한다. 다만 YOLO를 COCO 또는 Pascal VOC로 실제 export하려면 이미지 크기가 필요하므로 이미지가 연결되지 않은 레코드는 변환 실행 시 출력에서 제외될 수 있다. YOLO class mapping이 없으면 변환은 가능하지만 category name이 실제 클래스명이 아니라 `"0"`, `"1"` 같은 문자열 id가 될 수 있으므로 warning으로 표시된다.
...
대화형 변환에서 라벨 후보는 찾았지만 이미지 디렉터리를 찾지 못하면 COCO/VOC 출력에 필요한 이미지 크기를 계산할 수 없으므로 실행 계획을 만들지 않는다. 이 경우 원본 이미지를 workspace의 `data/raw`, `data/images`, `images` 중 하나에 두거나 프롬프트에 명시적 경로를 지정해야 한다.
...
| 클래스 매핑 파일 | Grounding DINO/SAM3 후보 클래스에 사용할 YOLO `data.yaml`, `dataset.yaml`, `classes.txt` | 선택 |
...
라벨 생성 탭도 대화형 작업과 동일하게 초기 실행에서는 specialist 재추론을 수행하지 않는다. 1차 결과 리포트를 확인한 뒤 결과 화면 아래의 `Specialist 재추론 1회 실행` 영역에서 advisor mode `none`, `low`, `high`, `both`를 선택해 검증 재추론을 이어서 실행한다.
...
Specialist 재추론은 report-only 검증 단계다. 1차 specialist 결과를 최종 라벨 후보로 유지하고, threshold/prompt/augmentation 설정을 조금 바꾼 specialist 결과와 bbox IoU agreement를 계산한다. 기본 object detection은 Grounding DINO 단독, 기본 segmentation은 Grounded-SAM2 pipeline(Grounding DINO + SAM2)을 사용한다. Advisor가 `low`, `high`, `both`이면 선택한 LLM은 설정 patch만 제안하며 클래스 목록, bbox, 최종 라벨은 직접 수정하지 못한다.
