...
| `custom_mapping` | `.json` 파일 또는 디렉터리 | 사용자 지정 mapping spec 기반 JSON 라벨 |
...
포맷별 Importer
  - YOLO / VOC / COCO
  - Vision JSONL / CSV / generic JSON / custom_mapping
        |
        v
...
`custom_mapping`은 표준 포맷으로 판별되지 않는 JSON 라벨을 사용자가 지정한 mapping spec으로 읽는 경로다. Streamlit 형식 변환 탭과 대화형 변환은 샘플 JSON을 분석해 `image_name_path`, `objects_path`, `label_path`, `bbox_path`, `bbox_format`, `bbox_unit` 등을 자동 제안할 수 있다. 객체 내부 필드는 `@.bbox`, `@.species`처럼 현재 object context 기준 경로를 사용한다.
...
- 같은 위치에서 YOLO 숫자 라벨과 다른 포맷의 실제 클래스명이 겹치면 실제 클래스명을 canonical label로 사용하고 `input_summary.merge.label_normalizations`에 기록한다.
- 숫자 class id와 클래스명의 대응이 하나로만 추론되면 같은 변환 배치의 YOLO-only 라벨에도 전파하고 `input_summary.merge.global_label_normalizations`에 기록한다.
- 그 외 같은 위치에서 클래스가 다르면 둘 다 보존하고 `input_summary.merge.conflicts`에 기록한다.
...
CSV와 generic JSON은 별도 image width/height 컬럼을 사용하지 않는다. 좌표 값이 `1.0`보다 크면 공통 `normalize_coordinate()`가 `0~1000` 스케일로 간주해 `1000`으로 나눈다. 따라서 임의 pixel 좌표를 가진 CSV/generic JSON을 정확히 변환하려면 먼저 normalized 또는 0~1000 좌표로 준비해야 한다.

custom_mapping은 매핑 스펙에 image width/height 경로와 `bbox_unit: "pixel"`을 포함할 수 있으므로, 라벨 JSON 안에 이미지 크기 정보가 있다면 pixel bbox도 정확히 정규화할 수 있다.
...
class id는 입력에서 확정한 canonical class list 순서로 부여된다. YOLO 입력에 `data.yaml`, `dataset.yaml`, 기타 YAML, `classes.txt`가 있으면 이 순서가 우선 보존된다. mapping 파일은 YOLO 라벨 파일 폴더에서 입력 루트까지 상위 디렉터리를 탐색하며 찾고, 다른 포맷에서만 발견된 클래스는 뒤에 추가된다.
...
YOLO 출력 class id는 importer가 만든 canonical class list를 따른다. 우선순위는 명시된 `--classes`의 YAML/TXT, YOLO 라벨 폴더부터 입력 루트까지의 `data.yaml`/`dataset.yaml`, 기타 YAML, `classes.txt`, 마지막으로 라벨에서 발견된 클래스 순서다. GT와 prediction을 비교할 때 양쪽 `data.yaml` 또는 `classes.txt`의 class 순서를 반드시 일치시켜야 한다.
