...
- 저비용 모델과 고성능 모델을 계층적으로 사용해 품질과 비용을 동시에 관리한다.
...
저용량 모델을 여러 번 호출해 같은 이미지에 대한 라벨 후보를 생성한다. 후보 간 기하학적 일관성을 task별 지표로 계산한다.

| Task | Consistency Metric |
| --- | --- |
...
불확실성은 consistency와 confidence를 결합해 계산한다.

```text
uncertainty_score = 1 - ((consistency_score + mean_confidence) / 2)
```

Consistency가 threshold보다 낮으면 고용량 모델을 호출하고, 그렇지 않으면 저용량 모델 결과를 최종 라벨로 채택한다.
...
- 저용량/고용량 모델 분리 및 동일 모델 사용 기본 차단
- consistency threshold 기반 high-model escalation
...
- 고성능 모델을 모든 샘플에 호출하지 않고, 불확실한 샘플에 집중할 수 있다.
- Ground Truth가 없는 상황에서도 반복 추론 consistency로 품질 판단 근거를 만들 수 있다.
...
- 고용량 모델이 호출되는 샘플이 많아지면 비용 절감 효과가 줄어든다.
- 복잡한 custom schema는 자동 변환보다 사용자 지정 mapping이 필요할 수 있다.
- Streamlit은 localhost 사용을 기본으로 하며 외부 공개 서비스 형태의 인증/권한 관리는 별도 설계가 필요하다.

## 8. Conclusion

본 프로젝트는 Agentic Workflow 기반 자동 라벨링 시스템을 통해 컴퓨터 비전 데이터 준비 과정의 라벨 생성, 포맷 변환, 검증, 오류 수정 안내를 통합했다. 계층적 LMM cascade는 self-consistency를 활용해 고성능 모델 호출을 선택적으로 수행하며, 다중 포맷 변환 구조는 실제 데이터셋에서 발생하는 class ID/class name 불일치 문제를 완화한다.

또한 Streamlit과 Windows `setup.exe` 배포를 고려한 구현 구조를 제공함으로써 연구용 프로토타입을 실제 사용자 도구로 확장할 수 있는 기반을 마련했다. 향후 실제 데이터셋 기반 정량 평가를 통해 cascade의 비용 절감률, 라벨 품질 향상, 사용자 수정 시간 감소 효과를 실험적으로 검증할 예정이다.

## 9. Future Work

- 실제 Ground Truth 데이터셋 기반 ablation study 수행
- low-only, high-only, cascade 방식의 비용·정확도 비교
...
5. Result table: low-only, high-only, cascade 비교 표

## One-Sentence Contribution

서로 다른 용량의 Vision LMM을 계층적으로 연결하고, 다중 라벨 포맷 변환과 사용자 수정 제안 리포트를 통합해 컴퓨터 비전 데이터 준비 과정을 자동화 가능한 workflow로 구현했다.
