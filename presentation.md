...
| Hierarchical Verification Agent | VLM fallback consistency 계산과 선택적 High LMM 검증, LMM 재생성 비교 결과 정리 | Verification result, review decision |
...
1차 Vision Specialist 결과를 기준으로 두고, 사용자가 선택한 Low/High LMM 재생성 결과를 비교한다. bbox 기반 태스크에서는 같은 클래스 bbox 간 IoU matching으로 self_consistency를 계산하고, 임계치 미달 이미지만 라벨 편집 큐로 전달한다. LMM 재생성 결과는 1차 라벨을 덮어쓰지 않고 `*_llm/low`, `*_llm/high` 폴더에 별도 저장되며, 사용자는 라벨 편집 시 1차 Vision 라벨 또는 LMM 재생성 라벨 중 편집 기준을 선택할 수 있다.
...
LMM 재생성 비교 경로에서는 최종 라벨을 자동 교체하지 않고 self_consistency와 검토 필요 여부를 리포트한다. Low/High LMM이 생성한 라벨은 별도 폴더에 저장하고, 임계치 미달 이미지는 라벨 에디터로 전달한다. 이때 사용자는 1차 Vision 라벨, Low LMM 라벨, High LMM 라벨 중 어떤 산출물을 기준으로 수정할지 선택할 수 있다.
...
현재 기능 검증과 발표 시연은 Streamlit UI를 기준으로 한다. Windows installer는 로컬 배포 경로이며, workspace와 환경 변수 설정을 공유한다. 대용량 specialist 의존성과 모델 weight는 기본 installer에 묶기보다 별도 specialist 배포판 또는 최초 실행 시 loader/cache 방식으로 다룬다.
...
| `desktop_app.py` | Windows desktop packaging/legacy GUI path |
...
| --- | --- | --- |
| Unit tests | 구현 회귀 테스트 개수 | 프로젝트 테스트 스위트로 검증 |
...
- Streamlit UI의 결과 차트와 포스터용 export 기능 강화

## 10. Acknowledgements

본 프로젝트는 컴퓨터 비전 데이터셋 구축 과정에서 발생하는 라벨링 비용, 포맷 불일치, 검증 자동화 문제를 해결하기 위한 연구 및 구현 과제로 수행되었다. 시스템 구현에는 Python 기반 workflow orchestration, Vision LMM API, Streamlit, 표준 라벨 포맷 변환 도구가 활용되었다.
